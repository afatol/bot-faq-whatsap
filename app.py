...
import requests
import os
from threading import Thread  # <--- Ferramenta para rodar em segundo plano
...
def enviar_mensagem_whatsapp(to: str, message: str):
    ...
    try:
        # Debug para garantir que estamos usando o token certo
        print(f"Tentando enviar para {to} com Token iniciando em: {WHATSAPP_TOKEN[:10]}...")
        resp = requests.post(url, headers=headers, json=payload)
        print(f"Status do Envio: {resp.status_code} - {resp.text}")
...
def tarefa_assincrona(text, phone):
    ...
    Esta função roda em uma linha separada (Thread).
    O Flask não fica esperando ela terminar.
    """
    try:
        print(f"--- [Thread] Iniciando processamento de IA para: {phone} ---")
        
        # 1. Busca na IA (Pode demorar o tempo que quiser aqui)
        ...
        print("--- [Thread] Ciclo finalizado com sucesso ---")
        
    except Exception as e:
        print(f"Erro critico na Thread: {e}")
...
def webhook():
    """Recebe a mensagem, lança a Thread e libera o servidor IMEDIATAMENTE"""
                ...
                for change in entry["changes"]:
                    ...
                    for message in messages:
                        ...
                        if phone and text:
                            ...
                            # Em vez de processar direto, nós lançamos a Thread:
                            thread = Thread(target=tarefa_assincrona, args=(text, phone))
                            thread.start()
                            
    # Responde "OK" instantaneamente para a Meta não dar Timeout
    return "OK", 200
